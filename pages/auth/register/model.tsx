...
> {
  ...
  render() {
    const { submiting, ui, t } = this.props;
    const { error, errorMessage, countries } = this.state;
    return (
      <>
        <Head>
          <title>
            {ui?.singularTextModel || t('Performer')} {t('Register')}
          </title>
        </Head>
        <div className="register-page">
          <div className="form-register-container">
            {errorMessage && (
              <Alert type="error" message={t('Error request')} banner />
            )}
            <RegisterForm
              singularTextModel={ui?.singularTextModel || t('Performer')}
...
export default connect(
  mapStateToProps,
  mapDispatchs
)(withTranslation('common')(PerformerRegisterPage));
