...
class PurchasedGalleriesPage extends PureComponent<IProps, IStates> {
  ...
  render() {
    ...
    return (
      <>
        <Head>
          <title>{t('My Purchased Galleries')}</title>
        </Head>
        <div className="main-profile-background">
          <PageHeader title={t('Purchased Gallery')} />
          <div className="purchased-videos-page pad40">
            {!loading && data?.length ? (
              ...
              <div className="pad20">
                {t('You have not purchased any galleries yet')}
              </div>
            )}
          </div>
        </div>
...
export default withTranslation('common')(PurchasedGalleriesPage);
