...
class ProductsPage extends PureComponent<IProps, IStates> {
  ...
  async infinityScroll() {
    try {
      ...
      const performerId = performer ? performer._id : '';
      const resp: IResponse<IDataResponse<IProduct>> =
  ...
  purchase(item: IProduct) {
    if (item.type === 'digital' && item.isBought) {
      return message.success(
        this.props.t(
          'You have bought this product, please check your email to get link'
        )
      );
    }
                ...
                >
                  <Row gutter={20}>
                    {ids && ids.length > 0 ? (
                      ...
                      <p className="no-items-found">{t('No product found')}.</p>
                    )}
                  </Row>
                </InfiniteScroll>
              )}
...
export default withRouter(
  connect(mapStates, mapDispatch)(withTranslation('common')(ProductsPage))
);
